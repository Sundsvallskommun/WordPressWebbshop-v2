(function($) {
	...
	$( document ).ready( function() {
		window.setTimeout( function() {
			var $number_select = $( 'select.js-search-raindance-number' );
			var $qty_input = $( 'input[name=quantity]' );
			if ( $number_select.length === 0 || $qty_input.length === 0 ) {
				return;
			}

			var _number = Number.init( $number_select.get(0), $qty_input.get(0) );
		}, 200 );
	} );
})(jQuery);